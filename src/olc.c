...
void show_olc_cmds( CHAR_DATA *ch, const struct olc_cmd_type *olc_table )
{
    ...
    {
        bprintf( buf, "%-15.15s", olc_table[cmd].name );
...
void redit( CHAR_DATA *ch, char *argument )
{
    ROOM_INDEX_DATA *pRoom;
    AREA_DATA *pArea;
    char arg[MAX_INPUT_LENGTH * 2]; /* was MSL */
    char command[MAX_INPUT_LENGTH];
    int  cmd;
    int  value;
...
void do_aedit( CHAR_DATA *ch, char *argument )
{
	...
	/* Security check by Rahl */
	if ( ch->pcdata->security != 9 ) {
		send_to_char( "You don't have the proper security"
			" to modify/create an area.\n\r", ch );
		return;
	}
...
{
    ROOM_INDEX_DATA *pRoom;
    char arg1[MAX_INPUT_LENGTH * 2]; /* was MSL */
...
{
    BUFFER *buf = buffer_new( MAX_INPUT_LENGTH );
    BUFFER *result = buffer_new( MAX_INPUT_LENGTH );    /* May need tweaking. */
    AREA_DATA *pArea;

    bprintf( result, "[%3s] [%-27s] (%-5s-%5s) [%-10s] %3s [%-10s]\n\r",
