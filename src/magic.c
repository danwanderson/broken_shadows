...
void do_cast( CHAR_DATA *ch, char *argument )
{
	...
	if ( IS_NPC( ch ) ) {
		mana = mana_cost( skill_table[sn].min_mana, 100 );
	} else {
    ...
    {
        ch->mana -= mana;
        //(*skill_table[sn].spell_fun) ( sn, ch->xp_level, ch, vo, target );
        (*skill_table[sn].spell_fun) ( sn, 10, ch, vo, target );
