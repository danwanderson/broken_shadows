...
char *get_curdate()
{
   ...
   time(&tm);
   now = *localtime(&tm);
   sprintf(thedate,"%02d/%02d/%04d",now.tm_mon+1,int (now.tm_mday),now.tm_year+1900);
...
void affect_join( CHAR_DATA *ch, AFFECT_DATA *paf )
{
    ...
    for ( paf_old = ch->affected; paf_old != NULL; paf_old = paf_old->next )
    {
        ...
        {
            int lvl = (paf->level += paf_old->level) / 2;
            paf->level = lvl;
...
/* RT changed so that WIZ_INVIS has levels */
    ...
    &&   char_getImmRank( ch ) < victim->invis_level )
        return FALSE;

    if ( !IS_NPC( victim ) 
    &&   IS_SET( victim->act, PLR_INCOGNITO )
    &&   char_getImmRank( ch ) < victim->incog_level 
