...
void boot_db(  )
{
     ...
     */
    {
        ...
        for ( ; ; )
        {
            ...
            for ( ; ; )
            {
               ...
               /*
                * changes below using skip_section by Ron Cole and Laurie
                * Zenner. Added here by Rahl
                */
               if ( word[0] == '$' ) break;
 /* OLC */     else if ( !str_cmp( word, "AREADATA" ) ) new_load_area(fpArea);
     ...
     */
    {
        ...
        save_boards( );
        load_disabled( );  /* load disabled command list */
        load_bans( );      /* load the ban list */
        load_songs( );     /* load the songs */
    }
...
void area_update( void )
{
    ...
    for ( pArea = area_first; pArea != NULL; pArea = pArea->next )
    {
        ...
        ||    pArea->age >= 31)
        {
            ...
            reset_area( pArea );
            /* added by Rahl */
            bprintf( buf, "%s has just been reset.", &pArea->name[16] );
            wiznet( buf->data, NULL, NULL, WIZ_RESETS, 0, 0 );
...
void reset_room( ROOM_INDEX_DATA *pRoom )
{
    ...
    /* added by Rahl - not sure if it's gonna stay or not.. */
    if ( pRoom->status >= 1 )
    {
        if ( pRoom->timer > 3 )
        {
    ...
    for ( pReset = pRoom->reset_first; pReset != NULL; pReset = pReset->next )
    {
        ...
        case 'O':
            ...
            pObj->cost = 0;
            if ( pObjIndex->count > 1 && IS_OBJ_STAT( pObj, ITEM_RARE ) )
                extract_obj( pObj );
            else
...
*/
            ...
            if ( pObj->pIndexData->count > 1 && IS_OBJ_STAT( pObj, ITEM_RARE ) )
                    extract_obj( pObj );
                else
...
/*
 * do_areas modified by Rahl so we can use page_to_char
 */
...
void do_memory( CHAR_DATA *ch, char *argument )
{
    ...
    bprintf( buf, "Affects %5d\n\r", top_affect    );
    send_to_char( buf->data, ch );
    bprintf( buf, "Areas   %5d\n\r", top_area      );
    send_to_char( buf->data, ch );
    bprintf( buf, "ExDes   %5d\n\r", top_ed        );
    send_to_char( buf->data, ch );
    bprintf( buf, "Exits   %5d\n\r", top_exit      );
    send_to_char( buf->data, ch );
    bprintf( buf, "Helps   %5d\n\r", top_help      );
    send_to_char( buf->data, ch );
    bprintf( buf, "Socials %5d\n\r", social_count  );
    send_to_char( buf->data, ch );
    bprintf( buf, "Mobs    %5d\n\r", top_mob_index );
    send_to_char( buf->data, ch );
    bprintf( buf, "(in use)%5d\n\r", mobile_count  );
    send_to_char( buf->data, ch );
    bprintf( buf, "Objs    %5d\n\r", top_obj_index );
    send_to_char( buf->data, ch );
    bprintf( buf, "Resets  %5d\n\r", top_reset     );
    send_to_char( buf->data, ch );
    bprintf( buf, "Rooms   %5d\n\r", top_room      );
    send_to_char( buf->data, ch );
    bprintf( buf, "Shops   %5d\n\r", top_shop      );
    send_to_char( buf->data, ch );
    bprintf( buf, "Clans   %5d\n\r", top_clan      );
    send_to_char( buf->data, ch );

    bprintf( buf, "Strings %5d strings of %7d bytes (max %d).\n\r",
        nAllocString, sAllocString, MAX_STRING );
    send_to_char( buf->data, ch );

    bprintf( buf, "Perms   %5d blocks  of %7d bytes.\n\r",
...
/*
 * Stick a lot fuzz on a number. -- Rahl
 */
...
/*
 * Skip a section. By Ron Cole and Laurie Zenner -- Added by Rahl
 */
void skip_section( FILE *fp, char *section )
{
    int number;
    ...
    if (!str_cmp( section, "CLANS" ) )
    {
        for( ; ; )
        {
            word = feof( fp ) ? "999" : fread_string_eol( fp );
            if ( word[0] == '9' && word[1] == '9' && word[2] == '9' )
                return;
        }
    }
