...
void show_flag_cmds( CHAR_DATA *ch, const struct flag_type *flag_table )
{
    ...
    for (flag = 0; flag_table[flag].name[0] != '\0'; flag++)
    {
        ...
        {
            bprintf( buf, "%-19.18s", flag_table[flag].name );
...
void show_skill_cmds( CHAR_DATA *ch, int tar )
{
    ...
    for (sn = 0; sn < MAX_SKILL; sn++)
    {
        ...
        {
            bprintf( buf, "%-19.18s", skill_table[sn].name );
            buffer_strcat( buf1, buf->data );
            if ( ++col % 4 == 0 )
                buffer_strcat( buf1, "\n\r" );
        }
...
void show_spec_cmds( CHAR_DATA *ch )
{
    ...
    {
        bprintf( buf, "%-19.18s", &spec_table[spec].spec_name[5] );
        buffer_strcat( buf1, buf->data );
        if ( ++col % 4 == 0 )
            buffer_strcat( buf1, "\n\r" );
    }
...
bool show_help( CHAR_DATA *ch, char *argument )
{
    ...
    if ( arg[0] == '\0' )
    {
        bprintf( buffer, "Syntax:  ? [command]\n\r\n\r" );
        buffer_strcat( buffer, "[command]  [description]\n\r" );
    ...
    for (cnt = 0; help_table[cnt].command[0] != '\0'; cnt++)
    {
          ...
          && !str_prefix( arg, help_table[cnt].command ) )
        {
            ...
            if ( help_table[cnt].structure == skill_table )
            {
                ...
                else if ( !str_prefix( spell, "other" ) )
                    show_skill_cmds( ch, TAR_CHAR_OTHER );
                else
...
AEDIT( aedit_show )
{
    ...
    bprintf( buf, "Name:     [%5d] %s\n\r", pArea->vnum, pArea->name );
    send_to_char( buf->data, ch );

#if 0  /* ROM OLC */
    bprintf( buf, "Recall:   [%5d] %s\n\r", pArea->recall,
    ...
    bprintf( buf, "Vnums:    [%d-%d]\n\r", pArea->lvnum, pArea->uvnum );
    send_to_char( buf->data, ch );

    bprintf( buf, "Age:      [%d]\n\r", pArea->age );
    send_to_char( buf->data, ch );

    bprintf( buf, "Players:  [%d]\n\r", pArea->nplayer );
    send_to_char( buf->data, ch );
...
AEDIT( aedit_builder )
{
    ...
    else
    {
        ...
        if (pArea->builders[0] != '\0' )
        {
            buffer_strcat( buf, pArea->builders );
            buffer_strcat( buf, " " );
        }
        buffer_strcat( buf, name );
        free_string( pArea->builders );
        pArea->builders = string_proper( str_dup( buf->data ) );
...
REDIT( redit_show )
{
    ...
    if ( pRoom->extra_descr )
    {
        ...
        buffer_strcat( buf1, "Desc Kwds:  [" );
        for ( ed = pRoom->extra_descr; ed; ed = ed->next )
        {
            buffer_strcat( buf1, ed->keyword );
            if ( ed->next )
                buffer_strcat( buf1, " " );
        }
        buffer_strcat( buf1, "]\n\r" );
    ...
    if ( fcnt )
    {
        ...
        end = strlen(buf1->data) - 1;
        buf1->data[end] = ']';
        buffer_strcat( buf1, "\n\r" );
    }
    else
        buffer_strcat( buf1, "none]\n\r" );

    buffer_strcat( buf1, "Objects:    [" );
    fcnt = FALSE;
    ...
    for ( door = 0; door < MAX_DIR; door++ )
    {
        ...
        if ( ( pexit = pRoom->exit[door] ) )
        {
            ...
            {
                bprintf( buf, "%s", pexit->description );
                buffer_strcat( buf1, buf->data );
            }
        }
    }
...
REDIT( redit_mreset )
{
    ...
    bprintf( output, "%s (%d) has been loaded and added to resets.\n\r"
        "There will be a maximum of %d loaded to this room.\n\r",
        capitalize( pMobIndex->short_descr ),
        pMobIndex->vnum,
        pReset->arg2 );
    send_to_char( output->data, ch );
...
REDIT( redit_oreset )
{
    ...
    && ( ( to_obj = get_obj_list( ch, arg2, pRoom->contents ) ) != NULL ) )
    {
        ...
        bprintf( output, "%s (%d) has been loaded into "
    ...
    if ( ( to_mob = get_char_room( ch, arg2 ) ) != NULL )
    {
        ...
        bprintf( output, "%s (%d) has been loaded "
...
/* It somehow fixed a bug in showing scroll/pill/potions too ?! */
        case ITEM_WEAPON:
            bprintf( buf, "[v0] Weapon class:   %s\n\r",
                     flag_string( weapon_class, obj->value[0] ) );
            send_to_char( buf->data, ch );
            bprintf( buf, "[v1] Number of dice: [%d]\n\r", obj->value[1] );
            send_to_char( buf->data, ch );
            bprintf( buf, "[v2] Type of dice:   [%d]\n\r", obj->value[2] );
            send_to_char( buf->data, ch );
            bprintf( buf, "[v3] Type:           %s\n\r",
                    flag_string( weapon_flags, obj->value[3] ) );
            send_to_char( buf->data, ch );
            bprintf( buf, "[v4] Special type:   %s\n\r",
...
OEDIT( oedit_show )
{
    ...
    bprintf( buf, "Level:       [%5d]\n\r", pObj->level );
    buffer_strcat( buf1, buf->data );

    bprintf( buf, "Wear flags:  [%s]\n\r",
    ...
    if ( pObj->extra_descr )
    {
        ...
        buffer_strcat( buf1, "Ex desc kwd: " );
    ...
    for ( cnt = 0, paf = pObj->affected; paf; paf = paf->next )
    {
        if ( cnt == 0 )
        {
            buffer_strcat( buf1, "Number Modifier Affects\n\r" );
            buffer_strcat( buf1, "------ -------- -------\n\r" );
        }
        bprintf( buf, "[%4d] %-8d %s\n\r", cnt,
            paf->modifier,
...
/* ROM values: */
    ...
    bprintf( buf, "Hitroll:     [%d]\n\r",
             pMob->hitroll );
    buffer_strcat( buf1, buf->data );

    bprintf( buf, "Hit dice:    [%2dd%-3d+%4d]\n\r",
             pMob->hit[DICE_NUMBER],
             pMob->hit[DICE_TYPE],
             pMob->hit[DICE_BONUS] );
    buffer_strcat( buf1, buf->data );

    bprintf( buf, "Damage dice: [%2dd%-3d+%4d]\n\r",
             pMob->damage[DICE_NUMBER],
             pMob->damage[DICE_TYPE],
             pMob->damage[DICE_BONUS] );
    buffer_strcat( buf1, buf->data );

    bprintf( buf, "Mana dice:   [%2dd%-3d+%4d]\n\r",
...
/* ROM values end */
    ...
    if ( pMob->pShop )
    {
        ...
        for ( iTrade = 0; iTrade < MAX_TRADE; iTrade++ )
        {
            ...
            {
                if ( iTrade == 0 ) {
                    buffer_strcat( buf1, "  Number Trades Type\n\r" );
                    buffer_strcat( buf1, "  ------ -----------\n\r" );
                }
                bprintf( buf, "  [%4d] %s\n\r", iTrade,
...
MEDIT( medit_race )
{
    ...
    if ( argument[0] == '?' )
    {
        ...
        for ( race = 0; race_table[race].name != NULL; race++ )
        {
            if ( ( race % 3 ) == 0 )
                send_to_char( "\n\r", ch );
            bprintf( buf, " %-15s", race_table[race].name );
            send_to_char( buf->data, ch );
        }
